...
public class ReportesController : Controller
{
    ...
    public async Task<IActionResult> Ventas(DateTime? desde, DateTime? hasta, int? idPelicula, string? cliente, int page = 1)
{
    ...
    if (!string.IsNullOrWhiteSpace(cliente))
        query = query.Where(d =>
            d.IdVentaNavigation.IdUsuarioNavigation.Nombres.Contains(cliente) ||
            d.IdVentaNavigation.IdUsuarioNavigation.Apellidos.Contains(cliente) ||
            d.IdVentaNavigation.IdUsuarioNavigation.Correo.Contains(cliente));

    query = query.OrderByDescending(d => d.IdVentaNavigation.FechaVenta);

    int totalRegistros = await query.CountAsync();
    int totalPaginas = Math.Max(1, (int)Math.Ceiling(totalRegistros / 10.0));
    page = Math.Max(1, Math.Min(page, totalPaginas));

    var resultados = await query.Skip((page - 1) * 10).Take(10).ToListAsync();
